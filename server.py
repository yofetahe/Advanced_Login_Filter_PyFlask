...
def login():
    ...
    if user and bcrypt.check_password_hash(user[0]['password'], request.form['password']):
        ...
        session['u_level'] = user[0]['user_level']
        session['u_hash'] = hash((user[0]['user_level'], user[0]['email']))
...
@app.route("/admin")
def admin():    
    if check_admin_level():
        mysql = connectToMySQL(DB_NAME)
        query = "SELECT * FROM users"
        usersList = mysql.query_db(query)
        return render_template("admin.html", usersList=usersList)
    else:
        flash("You are not allowed to access the admin page.", 'access_denied')
        return redirect(url_for("home"))

def check_admin_level():
    return session['u_level'] == 1 and hash((session['u_level'], session['email'])) == session['u_hash']

@app.route("/get_user_info_update_form")
def get_user_info_update_form():
    if check_admin_level():
        return render_template("user_info_update_form.html")
    else:
        flash("You are not allowed to access the admin page.", 'access_denied')
        return redirect(url_for("home"))
